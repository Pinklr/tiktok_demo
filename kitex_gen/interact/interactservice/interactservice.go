...
func isFavoriteHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(interact.IsFavoriteRequest)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(interact.InteractService).IsFavorite(ctx, req)
		if err != nil {
			return err
		}
		if err := st.SendMsg(resp); err != nil {
	...
	case *IsFavoriteArgs:
		success, err := handler.(interact.InteractService).IsFavorite(ctx, s.Req)
		if err != nil {
			return err
		}
...
func (p *IsFavoriteArgs) Unmarshal(in []byte) error {
	msg := new(interact.IsFavoriteRequest)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Req = msg
...
var IsFavoriteArgs_Req_DEFAULT *interact.IsFavoriteRequest

func (p *IsFavoriteArgs) GetReq() *interact.IsFavoriteRequest {
	if !p.IsSetReq() {
...
type IsFavoriteResult struct {
	Success *interact.IsFavoriteResponse
}

var IsFavoriteResult_Success_DEFAULT *interact.IsFavoriteResponse

func (p *IsFavoriteResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(interact.IsFavoriteResponse)
	}
	return p.Success.FastRead(buf, _type, number)
}
...
func (p *IsFavoriteResult) Unmarshal(in []byte) error {
	msg := new(interact.IsFavoriteResponse)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *IsFavoriteResult) GetSuccess() *interact.IsFavoriteResponse {
	if !p.IsSetSuccess() {
...
func (p *IsFavoriteResult) SetSuccess(x interface{}) {
	p.Success = x.(*interact.IsFavoriteResponse)
}
...
func (p *kClient) IsFavorite(ctx context.Context, Req *interact.IsFavoriteRequest) (r *interact.IsFavoriteResponse, err error) {
